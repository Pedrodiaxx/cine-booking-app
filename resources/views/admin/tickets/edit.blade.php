<x-admin-layout>
    <div class="max-w-3xl mx-auto">
        <h1 class="text-2xl font-bold mb-6">✏ Editar boleto: {{ $ticket->code }}</h1>
        ...
        <form action="{{ route('tickets.update', $ticket) }}" method="POST" class="space-y-4 bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
                ...
                <select name="movie_id" class="w-full border-gray-300 rounded shadow-sm">
                    @foreach ($movies as $movie)
                        <option value="{{ $movie->id }}"
                            {{ old('movie_id', $ticket->movie_id) == $movie->id ? 'selected' : '' }}>
                            {{ $movie->title }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="room_id" class="w-full border-gray-300 rounded shadow-sm">
                    @foreach ($rooms as $room)
                        <option value="{{ $room->id }}"
                            {{ old('room_id', $ticket->room_id) == $room->id ? 'selected' : '' }}>
                            {{ $room->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="datetime-local" name="show_time"
                       value="{{ old('show_time', \Illuminate\Support\Carbon::parse($ticket->show_time)->format('Y-m-d\TH:i')) }}"
                       class="w-full border-gray-300 rounded shadow-sm">
            </div>
